...
predict_start_date = '2024-8-20'
predict_end_data = '2024-10-30'

# Get the previous day
previous_day = pd.Timestamp(predict_start_date) - pd.Timedelta(1, unit='D')

# Convert back to string with the same format
previous_day_str = str(previous_day.date())

X_future = X.loc[predict_start_date:predict_end_data]
future_time = X_future.index
y_future = y_smoothed_kalman.loc[predict_start_date:predict_end_data]
X = X.loc['2019-01-01':previous_day_str]
y_smoothed_kalman = y_smoothed_kalman.loc['2019-01-01':previous_day_str]
...
plt.plot(future_time, y_future, label='Actual Value', color='blue')
